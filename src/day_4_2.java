...
//kódok="Qzmt-Zixmtkozy-Ivhz-343[asdfg]\n";
//kódok="abcxyz-1[asd]\n";
    String egySor="xxx";
    int id,origId;
    String checksum;
    ...
    int i=0;
    int sorokSzáma=0;
    char[] roomCodeTömb=new char[100];
    String dekódoltSor;
    do{
      //egy sor kinyerése
      egySor="";
      ...
      //részSztringek meghatározása
      //System.out.print(egySor+"*");
      sorokSzáma++;
      roomCode=egySor.split("\\d+\\[.+\\]")[0];
      origId=Integer.decode(egySor.substring(egySor.lastIndexOf("-")+1,egySor.lastIndexOf("[")));
      id=origId%26;//122-97-ig vannak a betűk, ez 26 db.  
      //System.out.println(id+"*");
      for (int j = 0; j < roomCodeTömb.length; j++) {
        roomCodeTömb[j]=(char)0;
      }
      egySor.getChars(0, roomCode.length(), roomCodeTömb, 0);
      //checksum=egySor.substring(egySor.lastIndexOf("[")+1, egySor.lastIndexOf("]"));
      //System.out.println(checksum+"*");
      for (int j = 0; (j < roomCodeTömb.length) && ((int)roomCodeTömb[j]>0); j++) {
        if(roomCodeTömb[j]=='-')
          roomCodeTömb[j]=' ';
        else if(((int)roomCodeTömb[j])+id>122){
          roomCodeTömb[j]=(char)(((int)roomCodeTömb[j])-26+id);
        }
        else
          roomCodeTömb[j]=(char)((int)(roomCodeTömb[j])+id);
      }
      dekódoltSor="";
      for (char c : roomCodeTömb) {
        dekódoltSor+=c;
      }
      //if (dekódoltSor.contains("North Pole")) {
      String xx=dekódoltSor.toLowerCase();
      //if (dekódoltSor.toLowerCase().contains("north pole")) {
      if(xx.contains("north")){
        System.out.println("Room code:"+roomCode);
        System.out.println("Orig id: "+origId);
        System.out.println(dekódoltSor);
        System.out.println("");
